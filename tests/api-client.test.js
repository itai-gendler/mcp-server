...
describe('API Client Integration', () => {
  ...
  test('should make API calls with correct parameters', async () => {
    ...
    const firstToolCallback = server.tool.mock.calls[0][2];
  ...
  test('should extract path, query, and body parameters correctly', async () => {
    ...
    const queueToolIndex = server.tool.mock.calls.findIndex(call => 
      call[0].includes('queue') && call[0].includes('idOrName'));
    
    if (queueToolIndex !== -1) {
      const queueToolCallback = server.tool.mock.calls[queueToolIndex][2];
      ...
      if (postToolIndex !== -1) {
        ...
        await postToolCallback({ name: 'test', description: 'test description' });
        ...
        if (axiosCallConfig.data) {
          ...
          expect(axiosCallConfig.data).toHaveProperty('description', 'test description');
        }
      }
    }
  });
